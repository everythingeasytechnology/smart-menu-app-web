...
import { Checkbox, Radio, Segmented, message, Input, Modal, Form } from "antd";
import {
  ArrowLeftOutlined,
  CheckOutlined,
  DownOutlined,
...
export const Route = createFileRoute("/$tableId/cart")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Your Cart — Rominus Pizza And Burger" },
      {
...
function CartPage() {
  ...
  const [tab, setTab] = useState<string>("Delivery Type");
  ...
  const placeOrderMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      navigate({ to: "/$tableId", params: { tableId } });
    },
    onError: (err: Error) => {
  ...
  if (cart.count === 0) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-page px-6 text-center">
        ...
        <Link to="/$tableId" params={{ tableId }} className="rounded-xl bg-brand-green px-5 py-3 font-bold text-primary-foreground">
          Browse menu
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-page pb-28">
      ...
      <header className="bg-background px-4 pt-4 pb-3">
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-3">
          <button type="button" onClick={() => navigate({ to: "/$tableId", params: { tableId } })} aria-label="Go back">
